...
export function ChatInterface({ messages, onSendMessage, isGenerating }: ChatInterfaceProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden flex flex-col h-[calc(100vh-12rem)] sticky top-24">
      {/* Header */}
      <div className="px-6 py-4 border-b border-slate-200 bg-slate-50">
        <div className="flex items-center gap-2">
          <MessageSquare className="w-5 h-5 text-slate-600" />
          <h3 className="text-slate-900">Refine Component</h3>
        </div>
      </div>
        ...
        {messages.length === 0 ? (
          <div className="text-center space-y-4 py-8">
            ...
            <div className="space-y-2">
              <p className="text-slate-600">Start refining your component</p>
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {refinementSuggestions.map((suggestion, index) => (
                  <button
                    ...
                    className="px-3 py-1 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition-colors text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {suggestion}
                  </button>
                ))}
